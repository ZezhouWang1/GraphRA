...
DATASET_NAME = 'ogbn-arxiv'  # 'ogbn-products', 'cora', 'pubmed'
K = 5   
NUM_LABELS = 40
MAX_LENGTH = 80
BATCH_SIZE = 32
...
def main():
    ...
    # 使用函数生成数据
    embs_path = 'Data/ogbn-arxiv/x_embs.pt'
    train_top_k_neighbors_path = 'Data/ogbn-arxiv/train_top_k_neighbors.json'
    # valid_top_k_neighbors_path = 'Data/valid_top_k_neighbors.json'
    test_top_k_neighbors_path='Data/ogbn-arxiv/test_top_k_neighbors.json'

    try:
    # 直接加载 .pt 文件
        train_data=torch.load("Instruction-arxiv/train_data.pt")
        # valid_data=torch.load("Instruction/valid_data.pt")
        test_data=torch.load("Instruction-arxiv/test_data.pt")
    ...
    # 如果文件不存在，则处理数据并保存
        ...
        torch.save(train_data, "Instruction-arxiv/train_data.pt")
        # torch.save(valid_data, "Instruction/valid_data.pt")
        torch.save(test_data, "Instruction-arxiv/test_data.pt")


    try:
    # 直接加载 .pt 文件
        train_instructions = torch.load("Instruction-arxiv/train_instructions.pt")
    except FileNotFoundError:
    # 如果文件不存在，则处理数据并保存
        ...
        torch.save(train_instructions, "Instruction-arxiv/train_instructions.pt")
    ...
    try:
    # 直接加载 .pt 文件
        test_instructions = torch.load("Instruction-arxiv/test_instructions.pt")
    except FileNotFoundError:
    # 如果文件不存在，则处理数据并保存
        ...
        torch.save(test_instructions, "Instruction-arxiv/test_instructions.pt")
